...
func Parse(doc []byte, kind AnsibleDocKind) (*AnsibleNode, error) {
	...
	switch kind {
	case PlaybookDoc:
		return parseAnsiblePlaybook(docNode)
	case RoleDoc:
		return parseTaskList(docNode.Children()[0])
	}
...
func parseAnsiblePlaybook(node yaml.YAMLNode) (*AnsibleNode, error) {
	...
	playbook.StartLine = listParent.Line() + 1
